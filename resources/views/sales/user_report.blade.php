...
@endphp
    ...
    <div class="d-flex align-items-center justify-content-center mb-3">
        <div class="mb-0 flex-grow-1">
            <div class="mb-0 h4"> <span class="fw-bold" dir="ltr">{{$user->name}}</span></div>
        </div>
        <x-back-btn href="{{ route('sales.users') }}" />
    </div>
    <x-card>
        ...
        <table class="table table-bordered">
            <tbody>
                <tr>
                    <td>@lang('Authority Name')</td>
                    <td>{{ $user->name }}</td>
                </tr>
                <tr>
                    <td>@lang('Total Items Sold')</td>
                    <td>{{ $user->total_items_sold }}</td>
                </tr>
                <tr>
                    <td>@lang('Total Sale')</td>
                    <td>{{ currency_format($user->total_sale) }}</td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="card" style="border-radius: 10px; border: 1px solid #dcdcdc; margin-bottom: 30px;">
            <div class="card-header" style="background-color: #f7f7f7; border-bottom: 2px solid #bdc3c7;">
                <h5 style="font-size: 1.25rem; font-weight: bold; color: #34495e; text-align: center;">Categorywise Expense Details</h5>
            </div>
            <div class="card-body">
                ...
                @else
                    <table class="table table-bordered" style="border-radius: 8px; margin-bottom: 20px;">
                        ...
                        <tbody style="border-top: 0;">
                            @foreach ($sales_category as $category)
                                <tr style="background-color: #f9fafb;">
                                    <td>{{ $category->category_name }}</td>
                                    <td class="text-center">{{ $category->total_qty }}</td>
                                    <td class="text-center">{{ currency_format($category->total_sales) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <div class="card" style="border-radius: 10px; border: 1px solid #dcdcdc; margin-bottom: 30px;">
            <div class="card-header" style="background-color: #f7f7f7; border-bottom: 2px solid #bdc3c7;">
                <h5 style="font-size: 1.25rem; font-weight: bold; color: #34495e; text-align: center;">Categorywise Sales Details</h5>
            </div>
            <div class="card-body">
                @if($sales_category->isEmpty())
                ...
                @else
                    ...
                    @foreach ($groupedSales as $categoryId => $categorySales)
                        <h5 style="font-size: 1.25rem; font-weight: bold; color: #34495e; margin-top: 20px; border-bottom: 2px solid #bdc3c7; text-align: center;">Category: {{ $categorySales->first()->category_name }}</h5>
                        <table class="table table-bordered" style="border-radius: 8px; margin-bottom: 20px;">
                            <thead style="background-color: #3498db; color: #fff; font-weight: bold; text-align: center;">
                                <tr>
                                    <th>@lang('Item')</th>
