...
@section('content')
    <div class="d-flex align-items-center justify-content-center mb-3">
        ...
        <div class="me-3">
            <a href="{{ route('sales.users') }}" class="btn btn-lg btn-success">Sale Report By User</a>
        </div>
                    ...
                    <tbody class="border-top-0">
                        @for ($i = 0; $i < $sales->count(); $i++)
                            <tr class="clickable-cell"
                                onclick="window.location='{{ route('sales.show', [Carbon\Carbon::parse($sales[$i]->date)->format('Y-m-d')]) }}?user_name={{ $sales[$i]->user_name }}&ts={{ $sales[$i]->total - $salesOrder[$i]->sum_discount }}';">
                                <td class="align-middle py-3 text-start" lang="en" dir="auto">
                                    {{ Carbon\Carbon::parse($sales[$i]->date)->format('ymd') }}
                                </td>
                                <td class="align-middle py-3">{{ $sales[$i]->user_name }}</td>
                                <td class="align-middle py-3 text-start" lang="en" dir="auto">
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('sales.filter') }}" method="GET" role="form">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="user_name" class="form-label">@lang('User')</label>
