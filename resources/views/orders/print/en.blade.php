    ...
    <div style="margin-bottom: 1.5rem">
        @foreach ($order->order_details as $detail)
            ...
            <div style="display: flex">
                <div> {{ $detail->quantity }}
                    ({{ $detail->price_type == 1 ? $detail->product->box_unit : $detail->product->cost_unit }})
                    *
                    {{ $detail->view_price }} </div>
                <div style="margin-left: auto">{{ $detail->view_total }}</div>
            </div>
        @endforeach
    </div>

    @if ($order->bucketStock > 0)
        <div style="display: flex;">
            <div>Bucket: {{ $order->bucketName }} - ({{ $order->bucketStock }} *
                {{ currency_format($order->bucketTotalPrice / $order->bucketStock) }})</div>
            <div style="margin-left: auto">{{ currency_format($order->bucketTotalPrice) }}</div>
        </div>
    @endif
    @if ($order->discount > 0)
        <div style="display: flex;">
