<form action="{{ isset($category) ? route('categories.update', $category) : route('categories.store') }}" method="POST"
    ...
    <div class="mb-3">
        <label for="minimum_stock" class="form-label">@lang('Minimum Stock')</label>
        <input type="text" name="minimum_stock" class="form-control @error('minimum_stock') is-invalid @enderror"
            id="minimum_stock" value="{{ old('minimum_stock', isset($category) ? $category->minimum_stock : '') }}">
        @error('minimum_stock')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
