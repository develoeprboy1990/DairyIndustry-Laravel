                    ...
                    <tr>
                                    ...
                                    <optgroup label="Cooling Room Products">
                                        @foreach ($coolingRoomProducts as $product)
                                            <option value="{{ $product->id }}"
                                                {{ old('item[]', isset($detail) ? $detail->product_id : '') == $product->id ? 'selected' : '' }}>
                                                {{ $product->name }}
                                            </option>
                                        @endforeach
                                    </optgroup>
                        ...
                        <td>
                            <input type="text" class=" form-control input-stock focus-select-text text-center"
                                name="quantity[]" value="{{ $detail->stock }}" value="{{ $detail->stock }}" required>
                        </td>
                        <td></td>
                    </tr>
                @endforeach
    ...
    <script>
        ...
        newItemBtn.addEventListener('click', function() {
            tbody.insertAdjacentHTML(
                'beforeend',
                '<tr><td><select label="Product Name" name="item[]" class="form-control"><option value = "" disabled selected > Select One </option> @if ($coolingRoomProducts->isNotEmpty()) <optgroup label = "Cooling Room Products" > @foreach ($coolingRoomProducts as $product) <option value = "{{ $product->id }}" {{ old('product_name', isset($car) ? $car->product_id : '') == $product->id ? 'selected' : '' }} > {{ $product->name }} </option>@endforeach </optgroup>@endif @if ($cat_warehouseProducts->isNotEmpty()) <optgroup label = "Plastics" >@foreach ($cat_warehouseProducts as $product)<option value = "{{ $product->id }}" {{ old('product_name', isset($car) ? $car->product_id : '') == $product->id ? 'selected' : '' }}>{{ $product->name }} </option> @endforeach </optgroup>@endif </select></td>' +
