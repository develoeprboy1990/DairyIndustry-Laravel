...
class ShankleeshController extends Controller
{
    ...
    public function edit(Shankleesh $shankleesh, string $day = '1'): View
    {
        ...
        $product = $shankleesh->product_id ? Product::find($shankleesh->product_id) : null;
        
        // $product = $shankleesh->product_id && $shankleesh->dairy_serial == $day ? Product::find($shankleesh->product_id) : null;
    ...
    public function update(Request $request, Shankleesh $shankleesh)
    {
        ...
        if ($shankleesh->product_id) {
            ...
            $product = Product::find($shankleesh->product_id);
            $product->update([
                'name' => $request->name,
            ...
            $shankleesh->product_id = $product->id; // Store the new product ID
        }else {
            // If next_phase checkbox is not checked, set product_id to null
            $shankleesh->product_id = null;
        }
