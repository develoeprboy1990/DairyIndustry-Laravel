...
use App\Http\Requests\CountryMilkRequest;
use Illuminate\View\View;

class CountryMilkController extends Controller
{
    /**
    ...
    {
        $countryMilks = CountryMilk::search($request->search_query)->orderBy('created_at', 'desc')->paginate(20);
        return view('country_milks.index', [
            'countryMilks' => $countryMilks
        ]);
    }
    ...
    {
        $countryMilks = CountryMilk::search($request->search_query)->orderBy('created_at', 'desc')->get();
        return view('country_milks.print', [
            'countryMilks' => $countryMilks
        ]);
    }
    ...
    public function create(): View
    {
        ...
        return view("country_milks.create", [
     ...
     *
     * @param  \App\Http\Requests\CountryMilkRequest  $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function store(CountryMilkRequest $request): RedirectResponse
    {
        $countryMilk = new CountryMilk();
        $countryMilk->type_of_milk = $request->type_of_milk;
        $countryMilk->qty_lp_powder = $request->qty_lp_powder;
        $countryMilk->qty_natural_milk = $request->qty_natural_milk;
        $countryMilk->qty_ACC_ghee = $request->qty_ACC_ghee;
        $countryMilk->qty_stabilizer = $request->qty_stabilizer;
        $countryMilk->qty_protein = $request->qty_protein;
        $countryMilk->qty_anti_mold = $request->qty_anti_mold;
        $countryMilk->qty_water = $request->qty_water;
        $countryMilk->final_quantity = $request->final_quantity;
        ...
        if ($request->next_phase) {
            ...
            // Attach the created product ID to the countryMilk
            $countryMilk->product_id = $product->id;
        ...
        $ingredient->quantity_of_LP_powder = $ingredient->quantity_of_LP_powder + ($request->qty_lp_powder ?? 0);
        $ingredient->quantity_of_milk = $ingredient->quantity_of_milk + ($request->qty_natural_milk ?? 0);
        $ingredient->quantity_of_ACC_ghee = $ingredient->quantity_of_ACC_ghee + ($request->qty_ACC_ghee ?? 0);
        $ingredient->quantity_of_stabilizer =  $ingredient->quantity_of_stabilizer + ($request->qty_stabilizer ?? 0);
        $ingredient->quantity_of_protin =  $ingredient->quantity_of_protin + ($request->qty_protein ?? 0);
        $ingredient->quantity_of_anti_mold =  $ingredient->quantity_of_anti_mold + ($request->qty_anti_mold ?? 0);
        $ingredient->quantity_of_water =  $ingredient->quantity_of_water + ($request->qty_water ?? 0);
     ...
     *
     * @param  \App\Models\CountryMilk  $countryMilk
     * @return \Illuminate\View\View
     */
    public function edit(CountryMilk $countryMilk): View
    {
         $milkCategory = Category::where('is_active', 1)->get();
        
        // Fetch the associated product, if it exists
        $product = $countryMilk->product_id
        ? Product::find($countryMilk->product_id)
        : null;
        ...
        return view("country_milks.edit", [
            'countryMilk' => $countryMilk,
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function update(CountryMilkRequest $request, CountryMilk $countryMilk): RedirectResponse
    {

        $old_countryMilk = CountryMilk::findOrFail($countryMilk->id);

        // ====save the Ingredients
        $ingredient = Ingredient::first();
        $ingredient->quantity_of_LP_powder = ($ingredient->quantity_of_LP_powder - $old_countryMilk->qty_lp_powder) + ($request->qty_lp_powder ?? 0);
        $ingredient->quantity_of_milk = ($ingredient->quantity_of_milk - $old_countryMilk->qty_natural_milk) + ($request->qty_natural_milk ?? 0);
        $ingredient->quantity_of_ACC_ghee = ($ingredient->quantity_of_ACC_ghee - $old_countryMilk->qty_ACC_ghee) + ($request->qty_ACC_ghee ?? 0);
        $ingredient->quantity_of_stabilizer = ($ingredient->quantity_of_stabilizer - $old_countryMilk->qty_stabilizer) + ($request->qty_stabilizer ?? 0);
        $ingredient->quantity_of_protin = ($ingredient->quantity_of_protin - $old_countryMilk->qty_protein) + ($request->qty_protein ?? 0);
        $ingredient->quantity_of_anti_mold = ($ingredient->quantity_of_anti_mold - $old_countryMilk->qty_anti_mold) + ($request->qty_anti_mold ?? 0);
        $ingredient->quantity_of_water =  ($ingredient->quantity_of_water - $old_countryMilk->qty_water) + ($request->qty_water ?? 0);
        // Save the updated record.
        $ingredient->save();
        // ============================
        ...
        if ($countryMilk->product_id) {
            ...
            $product = Product::find($countryMilk->product_id);
            ...
            $countryMilk->product_id = $product->id; // Store the new product ID
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
   public function destroy(CountryMilk $countryMilk): RedirectResponse
   {
       // Delete the associated product if it exists
       if ($countryMilk->product) {
           $countryMilk->product->delete(); // Delete the related Product
       }
