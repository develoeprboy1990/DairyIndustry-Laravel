...
class DairyIndustryController extends Controller
{
    ...
    public function edit(DairyIndustry $dairy, string $day = '1'): View
    {
        ...
        $product = $dairy->product_id ? Product::find($dairy->product_id) : null;
        
        // $product = $dairy->product_id && $dairy->dairy_serial == $day ? Product::find($dairy->product_id) : null;
    ...
    public function update(Request $request, DairyIndustry $dairy)
    {
        ...
        if ($dairy->product_id) {
            ...
            $product = Product::find($dairy->product_id);
            $product->update([
                'name' => $request->name,
            ...
            $dairy->product_id = $product->id; // Store the new product ID
        }else {
            // If next_phase checkbox is not checked, set product_id to null
            $dairy->product_id = null;
        }
        
         // $dairy['dairy_serial'] = $request->day;
