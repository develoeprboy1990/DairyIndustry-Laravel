...
class TommeController extends Controller
{
    ...
    public function edit(Tomme1 $tomme, string $day = '1'): View
    {
        ...
        $product = $tomme->product_id ? Product::find($tomme->product_id) : null;
        
        // $product = $tomme->product_id && $tomme->dairy_serial == $day ? Product::find($tomme->product_id) : null;
    ...
    public function update(Request $request, Tomme1 $tomme)
    {
        ...
        if ($tomme->product_id) {
            ...
            $product = Product::find($tomme->product_id);
            $product->update([
                'name' => $request->name,
            ...
            $tomme->product_id = $product->id; // Store the new product ID
        }else {
            // If next_phase checkbox is not checked, set product_id to null
            $tomme->product_id = null;
        }
