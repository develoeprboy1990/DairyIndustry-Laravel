...
use App\Http\Requests\DoubleCreamRequest;
use Illuminate\View\View;

class DoubleCreamController extends Controller
{
    /**
    ...
    {
        $doubleCreams = DoubleCream::search($request->search_query)->orderBy('created_at', 'desc')->paginate(20);
        return view('double_creams.index', [
            'doubleCreams' => $doubleCreams
        ]);
    }
    ...
    {
        $doubleCreams = DoubleCream::search($request->search_query)->orderBy('created_at', 'desc')->get();
        return view('double_creams.print', [
            'doubleCreams' => $doubleCreams
        ]);
    }
    ...
    public function create(): View
    {
        ...
        $creamCategory = Category::where('is_active', 1)->get();
        ...
        return view("double_creams.create", [
            'creamCategory' => $creamCategory, // Pass the 'mouneh' category
     ...
     *
     * @param  \App\Http\Requests\DoubleCreamRequest  $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function store(DoubleCreamRequest $request): RedirectResponse
    {

        $doubleCream = new DoubleCream();
        $doubleCream->type_of_cheese = $request->type_of_cheese;
        $doubleCream->quantity_of_cheese_whey = $request->quantity_of_cheese_whey;
        $doubleCream->quantity_of_cylinder_powder = $request->quantity_of_cylinder_powder;
        $doubleCream->quantity_of_calcium = $request->quantity_of_calcium;
        $doubleCream->final_quantity = $request->final_quantity;
        ...
        if ($request->next_phase) {
            ...
            // Attach the created product ID to the doubleCream
            $doubleCream->product_id = $product->id;
        ...
        $ingredient->quantity_of_whey =  $ingredient->quantity_of_whey + ($request->quantity_of_cheese_whey ?? 0);
     ...
     *
     * @param  \App\Models\DoubleCream  $doubleCream
     * @return \Illuminate\View\View
     */
    public function edit(DoubleCream $doubleCream): View
    {
         $creamCategory = Category::where('is_active', 1)->get();
        
        // Fetch the associated product, if it exists
        $product = $doubleCream->product_id
        ? Product::find($doubleCream->product_id)
        : null;
        ...
        return view("double_creams.edit", [
            'doubleCream' => $doubleCream,
            'creamCategory' => $creamCategory,
            'product'=> $product,
            'isNextPhaseChecked' => $isNextPhaseChecked,
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function update(DoubleCreamRequest $request, DoubleCream $doubleCream): RedirectResponse
    {
        $old_doubleCream = DoubleCream::findOrFail($doubleCream->id);

        // ====save the Ingredients
        $ingredient = Ingredient::first();
        $ingredient->quantity_of_whey =  ($ingredient->quantity_of_whey - $old_doubleCream->quantity_of_cheese_whey) + ($request->quantity_of_cheese_whey ?? 0);
        $ingredient->quantity_of_cylinder_powder =  ($ingredient->quantity_of_cylinder_powder - $old_doubleCream->quantity_of_cylinder_powder) + ($request->quantity_of_cylinder_powder ?? 0);
        $ingredient->quantity_of_calcium =  ($ingredient->quantity_of_calcium - $old_doubleCream->quantity_of_calcium) + ($request->quantity_of_calcium ?? 0);
        // Save the updated record.
        $ingredient->save();
        // ============================
        ...
        if ($doubleCream->product_id) {
            ...
            $product = Product::find($doubleCream->product_id);
            ...
            $doubleCream->product_id = $product->id; // Store the new product ID
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
   public function destroy(DoubleCream $doubleCream): RedirectResponse
   {
       // Delete the associated product if it exists
       if ($doubleCream->product) {
           $doubleCream->product->delete(); // Delete the related Product
       }
