...
use App\Http\Requests\CommercialMilkRequest;
use Illuminate\View\View;

class CommercialMilkController extends Controller
{
     /**
    ...
    {
        $commercialMilks = CommercialMilk::search($request->search_query)->orderBy('created_at', 'desc')->paginate(20);
        return view('commercial_milks.index', [
            'commercialMilks' => $commercialMilks
        ]);
    }
    ...
    {
        $commercialMilks = CommercialMilk::search($request->search_query)->orderBy('created_at', 'desc')->get();
        return view('commercial_milks.print', [
            'commercialMilks' => $commercialMilks
        ]);
    }
    ...
    public function create(): View
    {
        ...
        $milkCategory = Category::where('is_active', 1)->get();
        ...
        return view("commercial_milks.create", [
            'milkCategory' => $milkCategory, // Pass the 'mouneh' category
     ...
     *
     * @param  \App\Http\Requests\CommercialMilkRequest  $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function store(CommercialMilkRequest $request): RedirectResponse
    {

        $commercialMilk = new CommercialMilk();
        $commercialMilk->type_of_milk = $request->type_of_milk;
        $commercialMilk->quantity_of_LP_powder = $request->quantity_of_LP_powder;
        $commercialMilk->quantity_of_ACC_ghee = $request->quantity_of_ACC_ghee;
        $commercialMilk->quantity_of_starch = $request->quantity_of_starch;
        $commercialMilk->quantity_of_stabilizer = $request->quantity_of_stabilizer;
        $commercialMilk->quantity_of_sorbate = $request->quantity_of_sorbate;
        $commercialMilk->quantity_of_protin = $request->quantity_of_protin;
        $commercialMilk->quantity_of_anti_mold = $request->quantity_of_anti_mold;
        $commercialMilk->quantity_of_water = $request->quantity_of_water;
        $commercialMilk->final_quantity = $request->final_quantity;
        ...
        if ($request->next_phase) {
            ...
            // Attach the created product ID to the commercialMilk
            $commercialMilk->product_id = $product->id;
            ...
            $commercialMilk->product_id = null;
        }
        ...
        $ingredient->quantity_of_LP_powder = $ingredient->quantity_of_LP_powder + ($request->quantity_of_LP_powder ?? 0);
        ...
        $ingredient->quantity_of_sorbate = $ingredient->quantity_of_sorbate + ($request->quantity_of_sorbate ?? 0);
        $ingredient->quantity_of_protin = $ingredient->quantity_of_protin + ($request->quantity_of_protin ?? 0);
        $ingredient->quantity_of_anti_mold = $ingredient->quantity_of_anti_mold + ($request->quantity_of_anti_mold ?? 0);
     ...
     *
     * @param  \App\Models\CommercialMilk  $commercialMilk
     * @return \Illuminate\View\View
     */
    public function edit(CommercialMilk $commercialMilk): View
    {
         $milkCategory = Category::where('is_active', 1)->get();
        
        // Fetch the associated product, if it exists
        $product = $commercialMilk->product_id
        ? Product::find($commercialMilk->product_id)
        : null;
        ...
        return view("commercial_milks.edit", [
            'commercialMilk' => $commercialMilk,
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function update(CommercialMilkRequest $request, CommercialMilk $commercialMilk): RedirectResponse
    {

        $old_commercialMilk = CommercialMilk::findOrFail($commercialMilk->id);

        // ====save the Ingredients
        $ingredient = Ingredient::first();
        $ingredient->quantity_of_LP_powder = ($ingredient->quantity_of_LP_powder - $old_commercialMilk->quantity_of_LP_powder) + ($request->quantity_of_LP_powder ?? 0);
        $ingredient->quantity_of_ACC_ghee = ($ingredient->quantity_of_ACC_ghee - $old_commercialMilk->quantity_of_ACC_ghee) + ($request->quantity_of_ACC_ghee ?? 0);
        $ingredient->quantity_of_starch = ($ingredient->quantity_of_starch - $old_commercialMilk->quantity_of_starch) + ($request->quantity_of_starch ?? 0);
        $ingredient->quantity_of_stabilizer = ($ingredient->quantity_of_stabilizer - $old_commercialMilk->quantity_of_stabilizer) + ($request->quantity_of_stabilizer ?? 0);
        $ingredient->quantity_of_sorbate = ($ingredient->quantity_of_sorbate - $old_commercialMilk->quantity_of_sorbate) + ($request->quantity_of_sorbate ?? 0);
        $ingredient->quantity_of_protin = ($ingredient->quantity_of_protin - $old_commercialMilk->quantity_of_protin) + ($request->quantity_of_protin ?? 0);
        $ingredient->quantity_of_anti_mold = ($ingredient->quantity_of_anti_mold - $old_commercialMilk->quantity_of_anti_mold) + ($request->quantity_of_anti_mold ?? 0);
        $ingredient->quantity_of_water = ($ingredient->quantity_of_water - $old_commercialMilk->quantity_of_water) + ($request->quantity_of_water ?? 0);
        // Save the updated record.
        $ingredient->save();
        // ============================
        ...
        if ($commercialMilk->product_id) {
            ...
            $product = Product::find($commercialMilk->product_id);
            ...
            $commercialMilk->product_id = $product->id; // Store the new product ID
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
   public function destroy(CommercialMilk $commercialMilk): RedirectResponse
   {
       // Delete the associated product if it exists
       if ($commercialMilk->product) {
           $commercialMilk->product->delete(); // Delete the related Product
       }
   
       // Delete the MounehIndustry record
       $commercialMilk->delete();
