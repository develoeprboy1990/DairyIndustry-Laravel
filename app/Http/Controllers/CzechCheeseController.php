...
use App\Http\Requests\CzechCheeseRequest;
use Illuminate\View\View;

class CzechCheeseController extends Controller
{
    /**
    ...
    {
        $czechCheeses = CzechCheese::search($request->search_query)->orderBy('created_at', 'desc')->paginate(20);
        return view('czech_cheeses.index', [
            'czechCheeses' => $czechCheeses
        ]);
    }
    ...
    {
        $czechCheeses = CzechCheese::search($request->search_query)->orderBy('created_at', 'desc')->get();
        return view('czech_cheeses.print', [
            'czechCheeses' => $czechCheeses
        ]);
    }
    ...
    public function create(): View
    {
        ...
        $czechCategory = Category::where('is_active', 1)->get();
        ...
        return view("czech_cheeses.create", [
            'czechCategory' => $czechCategory, // Pass the 'mouneh' category
     ...
     *
     * @param  \App\Http\Requests\CzechCheeseRequest  $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function store(CzechCheeseRequest $request): RedirectResponse
    {
        $czechCheese = new CzechCheese();
        $czechCheese->type_of_cheese = $request->type_of_cheese;
        $czechCheese->quantity_of_milk = $request->quantity_of_milk;
        $czechCheese->quantity_of_swedish_powder = $request->quantity_of_swedish_powder;
        $czechCheese->quantity_of_tamara_ghee = $request->quantity_of_tamara_ghee;
        $czechCheese->quantity_of_starch = $request->quantity_of_starch;
        $czechCheese->quantity_of_stabilizer = $request->quantity_of_stabilizer;
        $czechCheese->obj_TC3 = $request->obj_TC3;
        $czechCheese->obj_704 = $request->obj_704;
        $czechCheese->quantity_of_salt = $request->quantity_of_salt;
        $czechCheese->quantity_of_cheese = $request->quantity_of_cheese;
        $czechCheese->quantity_of_water = $request->quantity_of_water;
        $czechCheese->final_quantity = $request->final_quantity;
        ...
        if ($request->next_phase) {
            ...
            // Attach the created product ID to the czechCheese
            $czechCheese->product_id = $product->id;
        ...
        $ingredient->quantity_of_swedish_powder = $ingredient->quantity_of_swedish_powder + ($request->quantity_of_swedish_powder ?? 0);
        $ingredient->quantity_of_tamara_ghee = $ingredient->quantity_of_tamara_ghee + ($request->quantity_of_tamara_ghee ?? 0);
        ...
        $ingredient->quantity_of_TC3 = $ingredient->quantity_of_TC3 + ($request->obj_TC3 ?? 0);
        $ingredient->quantity_of_704 =  $ingredient->quantity_of_704 + ($request->obj_704 ?? 0);
        $ingredient->quantity_of_salt =  $ingredient->quantity_of_salt + ($request->quantity_of_salt ?? 0);
        $ingredient->quantity_of_cheese =  $ingredient->quantity_of_cheese + ($request->quantity_of_cheese ?? 0);
     ...
     *
     * @param  \App\Models\CzechCheese  $czechCheese
     * @return \Illuminate\View\View
     */
    public function edit(CzechCheese $czechCheese): View
    {
         $czechCategory = Category::where('is_active', 1)->get();
        
        // Fetch the associated product, if it exists
        $product = $czechCheese->product_id
        ? Product::find($czechCheese->product_id)
        : null;
        ...
        return view("czech_cheeses.edit", [
            'czechCheese' => $czechCheese,
            'czechCategory' => $czechCategory,
            'product'=> $product,
            'isNextPhaseChecked' => $isNextPhaseChecked,
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function update(CzechCheeseRequest $request, CzechCheese $czechCheese): RedirectResponse
    {

        $old_czechCheese = CzechCheese::findOrFail($czechCheese->id);

        // ====save the Ingredients
        $ingredient = Ingredient::first();
        $ingredient->quantity_of_milk = ($ingredient->quantity_of_milk - $old_czechCheese->quantity_of_milk) + ($request->quantity_of_milk ?? 0);
        $ingredient->quantity_of_swedish_powder = ($ingredient->quantity_of_swedish_powder - $old_czechCheese->quantity_of_swedish_powder) + ($request->quantity_of_swedish_powder ?? 0);
        $ingredient->quantity_of_tamara_ghee = ($ingredient->quantity_of_tamara_ghee - $old_czechCheese->quantity_of_tamara_ghee) + ($request->quantity_of_tamara_ghee ?? 0);
        $ingredient->quantity_of_starch = ($ingredient->quantity_of_starch - $old_czechCheese->quantity_of_starch) + ($request->quantity_of_starch ?? 0);
        $ingredient->quantity_of_stabilizer = ($ingredient->quantity_of_stabilizer - $old_czechCheese->quantity_of_stabilizer) + ($request->quantity_of_stabilizer ?? 0);
        $ingredient->quantity_of_TC3 = ($ingredient->quantity_of_TC3 - $old_czechCheese->obj_TC3) + ($request->obj_TC3 ?? 0);
        $ingredient->quantity_of_704 =  ($ingredient->quantity_of_704 - $old_czechCheese->obj_704) + ($request->obj_704 ?? 0);
        $ingredient->quantity_of_salt =  ($ingredient->quantity_of_salt - $old_czechCheese->quantity_of_salt) + ($request->quantity_of_salt ?? 0);
        $ingredient->quantity_of_cheese =  ($ingredient->quantity_of_cheese - $old_czechCheese->quantity_of_cheese) + ($request->quantity_of_cheese ?? 0);
        $ingredient->quantity_of_water =  ($ingredient->quantity_of_water - $old_czechCheese->quantity_of_water) + ($request->quantity_of_water ?? 0);
        // Save the updated record.
        $ingredient->save();
        // ============================
        ...
        if ($czechCheese->product_id) {
            ...
            $product = Product::find($czechCheese->product_id);
            ...
            $czechCheese->product_id = $product->id; // Store the new product ID
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
   public function destroy(CzechCheese $czechCheese): RedirectResponse
   {
       // Delete the associated product if it exists
       if ($czechCheese->product) {
           $czechCheese->product->delete(); // Delete the related Product
       }
